...
class VectorNavNode(Node):
    ...
    def setup_publishers(self):
        ...
        self.ins_vel_body_publisher = self.create_publisher(TwistWithCovarianceStamped, 'vectornav/vel_body', 10)
        self.ins_vel_ned_publisher = self.create_publisher(TwistWithCovarianceStamped, 'vectornav/vel_ned', 10)
        self.orientation_ned_publisher = self.create_publisher(Odometry, 'vectornav/orientation_odometry', 10)
    ...
    def packet_handling(self):
        while self._thread_running and rclpy.ok():
            ...
            if compositeData.matchesMessage(self.vn_sensor.binaryOutput1Register):
                ...
                # frame id is world_ned if NED 
                # child frame is the frame on the unit
                orientation_msg = Odometry()
                orientation_msg.header.stamp = ros_timestamp
                orientation_msg.header.frame_id = self.ned_fram_id
                orientation_msg.header.child_frame_id = self.vn_config.frame_id
